...
def tracker(request):
    prod = request.POST.get('product', 'default')
    currency = request.POST.get('currency','default')
    min = request.POST.get('min','default')
    max = request.POST.get('max','default')
    NAME = prod
    CURRENCY = '€'
    max_price = max
    min_price = min
    ...
    print(data)
    with open(f'tracker/reports/{NAME}.json', 'r') as myfile:
        datas=myfile.read()


    return render(request,'home.html',{'data':data,'datas':datas})
